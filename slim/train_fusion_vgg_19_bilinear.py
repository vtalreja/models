...
# This is the file for training only the joint representation layer for the bilinear architecture using face and iris checkpoints
...
tf.app.flags.DEFINE_string(
	'train_dir', '/media/veerut/full/TensorFlow/Joint/Joint_CheckPoints_2013_overlap_vgg_19_joint_tf_record_no_bn_fc_layer_128_bilinear/',
...
tf.app.flags.DEFINE_string(
	'dataset_name_joint', 'wvu_joint_iris_and_face_overlap_2013_no_repeat', 'The name of the dataset to load.')

tf.app.flags.DEFINE_string(
	'dataset_dir_joint', '/media/veerut/full/TensorFlow/Joint/Joint_Iris_and_Face_Train_Data_2013_overlap_no_repeats_shuffle_TfRecords', 'The directory where the dataset files are stored.')

tf.app.flags.DEFINE_string(
	'model_name_iris', 'vgg_19_iris_bilinear', 'The name of the architecture to train.')
...
tf.app.flags.DEFINE_string(
	'model_name_face', 'vgg_19_face_bilinear', 'The name of the architecture to train.')
...
tf.app.flags.DEFINE_string(
	'model_name_joint', 'vgg_19_joint_128_bilinear', 'The name of the architecture to train.')
...
tf.app.flags.DEFINE_string(
	'checkpoint_path_iris', '/media/veerut/full/TensorFlow/Iris/CheckPoints_Train_Logs_bilinear_iris_2013_overlap_30_ep_128_fc/model.ckpt-10622',
...
tf.app.flags.DEFINE_string(
	'checkpoint_path_face', '/media/veerut/full/TensorFlow/Face/CheckPoints_Train_Logs_bilinear_face_2013_overlap_30_ep_128_fc/model.ckpt-24534',
	'The path to a checkpoint from which to fine-tune.')
...
tf.app.flags.DEFINE_string(
	'trainable_scopes_joint', 'vgg_joint/fc_joint,vgg_joint/fc_joint_r,vgg_joint/fc8_joint',
	'Comma-separated list of scopes to filter the set of variables to train.'
	'By default, None would train all the variables.')
...
def _get_init_op():
	...
	tf.logging.info('Fine-tuning from face checkpoint %s and iris checkpoint %s' % (checkpoint_path_face,checkpoint_path_iris))
...
def _get_variables_to_train():
	...
	if ((FLAGS.trainable_scopes_iris is None) and (FLAGS.trainable_scopes_face is None) and (FLAGS.trainable_scopes_joint is None)):
	   ...
	   #     FLAGS.model_name_iris,
		...
		with tf.device(deploy_config.inputs_device()):
			...
			batch_queue_joint = slim.prefetch_queue.prefetch_queue(
				[images_iris, labels_iris, images_face, labels_face], capacity=2 * deploy_config.num_clones)
		...
		clones = model_deploy.create_clones(deploy_config, clone_fn, [batch_queue_joint])
		...
		summaries.add(tf.image_summary('face',images_face,max_images=15,name='Face_images'))
		summaries.add(tf.image_summary('iris',images_iris,max_images=15,name='Iris_images'))
		...
		with tf.device(deploy_config.optimizer_device()):
			learning_rate = _configure_learning_rate(dataset_joint.num_samples, global_step)
